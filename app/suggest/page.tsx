...
function getUpdatedTime(updatedAt?: string) {
  if (!updatedAt) return "—";
  return new Date(updatedAt).toLocaleTimeString("cs-CZ", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
}
...
function getStatusBadgeClasses(state: CrawlState | null) {
  ...
  if (state.status === "running") return "border-emerald-200 bg-emerald-50 text-emerald-700";
...
function getEngineTheme(engine: "seznam" | "google") {
  if (engine === "seznam") {
    return {
      dot: "bg-red-500", softBg: "bg-red-50/50", panelBorder: "border-red-100",
      topTint: "from-red-50 to-white", progress: "from-red-500 to-orange-500",
      progressSoft: "bg-red-100/60", pill: "border-red-100 bg-red-50 text-red-700",
    };
  }
  return {
    dot: "bg-blue-500", softBg: "bg-blue-50/50", panelBorder: "border-blue-100",
    topTint: "from-blue-50 to-white", progress: "from-blue-500 to-cyan-500",
    progressSoft: "bg-blue-100/60", pill: "border-blue-100 bg-blue-50 text-blue-700",
  };
}

function StatMiniCard({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
      <div className="text-[10px] uppercase tracking-[0.18em] text-zinc-500">{label}</div>
      <div className="mt-2 text-[32px] font-semibold leading-none tracking-tight text-zinc-950">{value}</div>
...
function TopMetric({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
      ...
      <div className="mt-3 text-4xl font-semibold leading-none tracking-tight text-zinc-950">{value}</div>
      {hint ? <div className="mt-2 text-sm text-zinc-500">{hint}</div> : null}
    </div>
  );
}
...
function DetailRow({ label, value, emphasize = false }: { label: string; value: string; emphasize?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <span className={"text-sm " + (emphasize ? "font-semibold text-zinc-950" : "font-medium text-zinc-900")}>{value}</span>
    </div>
  );
}
...
function RecentPhraseItem({ phrase }: { phrase: string }) {
  return (
    <div className="rounded-xl border border-zinc-100 bg-zinc-50 px-3 py-2 text-sm text-zinc-700 transition hover:border-zinc-200 hover:bg-white">{phrase}</div>
  );
}

function SourcePanel({ title, engine, count, state, latest, recent }: {
  title: string; engine: "seznam" | "google"; count: number; state: CrawlState | null; latest: string; recent: string[];
}) {
  ...
  return (
    <section className={"overflow-hidden rounded-[32px] border " + theme.panelBorder + " bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)]"}>
      <div className={"bg-gradient-to-b " + theme.topTint + " px-6 py-6 md:px-8 md:py-7"}>
          ...
          <div className="min-w-0">
            ...
            <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-zinc-500">{latest ? "Poslední zachycená fráze: " + latest : "Zatím bez nových frází"}</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={"inline-flex items-center rounded-full border px-3 py-1.5 text-xs font-medium " + theme.pill}>Hloubka {currentDepth}</span>
              </div>
            </div>
          </div>
        </div>
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
          <div>
            ...
            <div className="mt-2 text-6xl font-semibold leading-none tracking-tight text-zinc-950 md:text-7xl">{formatNumber(count)}</div>
            <div className="mt-8">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <div className="text-base font-medium text-zinc-950">Průběh crawleru</div>
                  <div className="mt-1 text-sm text-zinc-500">Hloubka {currentDepth} — {formatNumber(processed)} / {formatNumber(queueSize)}</div>
                </div>
              ...
              <div className="mt-4 h-2.5 overflow-hidden rounded-full bg-zinc-100">
                <div className={"h-full rounded-full bg-gradient-to-r " + theme.progress + " transition-all duration-700"} style={{ width: progress + "%" }} />
              </div>
            </div>
          ...
          <div className="flex h-full flex-col rounded-[28px] border border-zinc-200 bg-zinc-50/80 p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex shrink-0 flex-col items-end">
                ...
                <div className="mt-2 rounded-2xl border border-zinc-200 bg-white px-3 py-2 font-mono text-sm font-medium text-zinc-900 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-zinc-500">{recent.length > 0 ? recent.length + " položek" : "bez dat"}</div>
            </div>
        ...
        <header className="rounded-[32px] border border-zinc-200 bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,0.03)] md:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
              <Link href="/" className="inline-flex h-10 items-center rounded-full border border-zinc-200 bg-white px-4 text-sm font-medium text-zinc-700 transition hover:bg-zinc-50 hover:text-zinc-950">← Zpět</Link>
              <div>
                <div className="flex flex-wrap items-center gap-3">
                  <h1 className="text-3xl font-semibold tracking-tight text-zinc-950 md:text-4xl">Crawler Dashboard</h1>
                  <span className={"inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium " + (activeCount > 0 ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-zinc-200 bg-zinc-50 text-zinc-600")}>{activeCount > 0 ? activeCount + " aktivní" : "Standby"}</span>
                </div>
                <p className="mt-2 text-sm text-zinc-500">Přehled sběru návrhů v reálném čase.</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:w-auto">
              <div className="rounded-3xl border border-zinc-200 bg-zinc-50 px-5 py-4 text-right">
                <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Aktualizace</div>
                <div className="mt-2 text-xl font-semibold leading-none text-zinc-950">každé 3 s</div>
              </div>
              <div className="rounded-3xl border border-zinc-200 bg-zinc-50 px-5 py-4 text-right">
                <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Projekt</div>
                <div className="mt-2 text-xl font-semibold leading-none text-zinc-950">good-inventions.work</div>
              </div>
            </div>
          </div>
        </header>
        ...
        <section className="mt-6 rounded-[32px] border border-zinc-200 bg-white p-6 shadow-[0_1px_2px_rgba(0,0,0,0.03)] md:p-8">
          <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr] xl:items-center">
            <div>
              ...
              <h2 className="mt-4 text-5xl font-semibold leading-none tracking-tight text-zinc-950 md:text-6xl">{formatNumber(total)} unikátních frází</h2>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <TopMetric label="Seznam.cz" value={formatNumber(seznamCount)} hint={getStatusText(seznamState)} />
              <TopMetric label="Google.cz" value={formatNumber(googleCount)} hint={getStatusText(googleState)} />
              <TopMetric label="Aktivních" value={String(activeCount)} hint={activeCount > 0 ? "crawluje" : "žádný aktivní"} />
            </div>
          </div>
        </section>

        <div className="mt-6 space-y-6">
          <SourcePanel title="Seznam.cz" engine="seznam" count={seznamCount} state={seznamState} latest={seznamLatest} recent={seznamRecent} />
          <SourcePanel title="Google.cz" engine="google" count={googleCount} state={googleState} latest={googleLatest} recent={googleRecent} />
        </div>
