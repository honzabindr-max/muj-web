...
function getEngineTheme(engine: "seznam" | "google") {
  if (engine === "seznam") {
    return { dot: "bg-red-500", panelBorder: "border-red-100", panelGlow: "shadow-[0_1px_2px_rgba(0,0,0,0.03),0_0_0_1px_rgba(239,68,68,0.03)]", topTint: "from-red-50 via-white to-white", softBlock: "bg-red-50/45", progress: "#ef4444", progressTrack: "#fee2e2", accentText: "text-red-600", miniPill: "border-red-100 bg-red-50 text-red-700", sparkFill: "rgba(239,68,68,0.10)" };
  }
  return { dot: "bg-blue-500", panelBorder: "border-blue-100", panelGlow: "shadow-[0_1px_2px_rgba(0,0,0,0.03),0_0_0_1px_rgba(59,130,246,0.03)]", topTint: "from-blue-50 via-white to-white", softBlock: "bg-blue-50/45", progress: "#0ea5e9", progressTrack: "#dbeafe", accentText: "text-blue-600", miniPill: "border-blue-100 bg-blue-50 text-blue-700", sparkFill: "rgba(14,165,233,0.10)" };
}
...
function DetailRow({ label, value, emphasize = false }: { label: string; value: React.ReactNode; emphasize?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <span className="text-sm text-zinc-500">{label}</span>
      <span className={"text-sm tabular-nums " + (emphasize ? "font-semibold text-zinc-950" : "font-medium text-zinc-900")}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="rounded-[28px] border border-zinc-200 bg-zinc-50/80 p-5">
      <div className="flex items-center justify-between gap-3">
        <div><div className="text-base font-medium text-zinc-950">Live feed</div><div className="mt-1 text-sm text-zinc-500">Poslední zachycené fráze v živém proudu.</div></div>
        <div className="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-600"><span className={"h-2 w-2 rounded-full " + dot + " animate-pulse"} />{label}</div>
      </div>
...
function SourcePanel({ title, engine, count, total, state, latest, recent }: { title: string; engine: "seznam" | "google"; count: number; total: number; state: CrawlState | null; latest: string; recent: string[] }) {
              ...
              <div className="flex h-full flex-col rounded-[28px] border border-zinc-200 bg-zinc-50/80 p-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex shrink-0 flex-col items-end">
                    ...
                    <div className="mt-2 rounded-2xl border border-zinc-200 bg-white px-3 py-2 font-mono text-sm font-medium text-zinc-900 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">{state?.current_prefix ? '"' + state.current_prefix + '"' : "—"}</div>
                  </div>
                </div>
        ...
        <header className="rounded-[32px] border border-zinc-200 bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,0.03)] md:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
              <Link href="/" prefetch={false} className="inline-flex h-10 items-center rounded-full border border-zinc-200 bg-white px-4 text-sm font-medium text-zinc-700 transition hover:bg-zinc-50 hover:text-zinc-950">← Zpět</Link>
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-3">
                  <h1 className="text-[clamp(2rem,4vw,3.5rem)] font-semibold tracking-tight text-zinc-950">Crawler Dashboard</h1>
                  <span className={"inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium " + (activeCount > 0 ? "border-emerald-200 bg-emerald-50 text-emerald-700 shadow-[0_0_0_4px_rgba(16,185,129,0.06)]" : "border-zinc-200 bg-zinc-50 text-zinc-600")}>{activeCount > 0 ? activeCount + " aktivní" : "Standby"}</span>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:w-auto">
              <div className="rounded-3xl border border-zinc-200 bg-zinc-50 px-5 py-4 text-right">
                ...
                <div className="mt-2 text-[clamp(1rem,2vw,1.35rem)] font-semibold leading-none text-zinc-950">každé 3 s</div>
              </div>
              <div className="rounded-3xl border border-zinc-200 bg-zinc-50 px-5 py-4 text-right">
        ...
        <section className="mt-6 rounded-[32px] border border-zinc-200 bg-white p-6 shadow-[0_1px_2px_rgba(0,0,0,0.03)] md:p-8">
          <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr] xl:items-center">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <TopMetric label="Seznam.cz" value={<AnimatedNumber value={seznamCount} />} hint={getStatusText(seznamState)} />
              <TopMetric label="Google.cz" value={<AnimatedNumber value={googleCount} />} hint={getStatusText(googleState)} />
              <TopMetric label="Aktivních" value={<AnimatedNumber value={activeCount} />} hint={activeCount > 0 ? "crawluje" : "žádný aktivní"} />
            </div>
          </div>
        </section>
