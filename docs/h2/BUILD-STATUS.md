...
**Aktuální slice:** BUILD-04 — Unified ingestion, AT GREEN lokálně (68/68 nových testů, 114/114 v celém repu), [PR #18](https://github.com/honzabindr-max/muj-web/pull/18) otevřen, branch `build/h2-build-04-unified-ingestion` zatím NEMERGNUTÁ. Obsahuje novou migraci `0014_telegram_rejected_audit_event.sql` — **slice není uzavřený**, dokud nedostane GO na merge + přidání env proměnných (`H2_TELEGRAM_WEBHOOK_SECRET`, `H2_TELEGRAM_OWNER_USER_ID`) do Vercelu a dokud není přímým dotazem na `_h2_migrations` ověřeno, že 0014 proběhla na production i preview větvi. BUILD-01 (PR #11), BUILD-02 (PR #12+#13), BUILD-03 (PR #14), BUILD-03A (PR #15) a hotfix (PR #17) jsou MERGED.
...
## BUILD-04 — Unified ingestion (AT GREEN lokálně, NEMERGNUTO)
...
- `h2/db/migrations/0014_telegram_rejected_audit_event.sql` — rozšiřuje `identity_audit_events_event_type_check` o `TELEGRAM_MESSAGE_REJECTED_UNKNOWN_SENDER`, aditivní. **Zatím neaplikováno na production ani preview — čeká na GO.**
...
**Zbývá pro uzavření slicu (GO od Honzíka):**
1. merge branch `build/h2-build-04-unified-ingestion` do `main`,
2. přidat `H2_TELEGRAM_WEBHOOK_SECRET` + `H2_TELEGRAM_OWNER_USER_ID` do Vercelu (production + preview),
3. aplikovat migraci 0014 na production i preview `h2-runtime` a ověřit přímým dotazem na `_h2_migrations`,
4. Honzíkovo `setWebhook` proti Telegram API reálným tokenem.
...
| BUILD-04 | Unified ingestion | AT GREEN lokálně — NEMERGNUTO, čeká na GO (merge + env + migrace 0014) | AT-01, AT-02, AT-48, AT-61 | [PR #18](https://github.com/honzabindr-max/muj-web/pull/18), branch `build/h2-build-04-unified-ingestion`; viz sekce výše |
