...
7. **BUILD-11 plán — NAPSÁN, ČEKÁ NA SCHVÁLENÍ, NEIMPLEMENTOVÁNO.** [docs/h2/BUILD-11-PLAN.md](./BUILD-11-PLAN.md) — 7 rozhodnutí: (1) trigger přes `after()` v ingest routách po vzoru §4.2 a BUILD-05-PLAN.md Rozhodnutí 2, (2) voice→text handoff fix (`readMessageText()` přijme `payload_type IN ('TEXT','VOICE')`), (3) retry taxonomie (rozpad `ANTHROPIC_HTTP_ERROR`, `retry-after` header, neretryovatelné třídy okamžitá karanténa), (4) **nová migrace** — `responses.owner_control_epoch` sloupec + delivery-time kontrola proti Pravidlu 10, (5) quarantine notice delivery seam (BUILD-05-PLAN.md Rozhodnutí 3) — samostatný mechanismus, ne `response_deliveries`, (6) Telegram `sendMessage` + web polling delivery mechanismus, (7) `MANUALLY_CLEARED` (PR #35) aplikační pořadí. AT-10 a §4.4/§4.5/§8.1 ověřeny živě v Notionu (ne jen parafrázovány). Rozhodnutí 4 a 5 vyžadují GO na migraci, zbytek je implementační návrh k Honzíkově kontrole.

**Shrnutí závislostí (aktualizace 2026-09-04):** #36 (tolerantní parser) MERGED. #38 (Structured Outputs) MERGED a nasazeno (merge `4709c13`, deploy `dpl_GLJ8YGTZJbVjBNYinetSFse5WBMa`, READY). Zbývá: #37 (re-auth, čeká na CI+GO) → ruční aktivace **DRAFT v7** (`id=9483ad27-ac71-4a34-a29a-8682445d331c` — **NE v6**, viz bod 1 výše) (Honzíkův krok, vyžaduje prohlížeč) → **[docs/h2/BUILD-11-PLAN.md](./BUILD-11-PLAN.md) — NÁVRH, čeká na Honzíkovo schválení celého plánu, NEIMPLEMENTOVÁNO** (trigger, voice payload_type fix, retry taxonomie, `owner_control_epoch` kontrola/Pravidlo 10, quarantine notice seam, PR [#35](https://github.com/honzabindr-max/muj-web/pull/35) `MANUALLY_CLEARED` pořadí).
