...
**OPERATIONAL_EXTRACTION certifikace — PŘIPRAVENO, NESPUŠTĚNO proti Haiku (2026-09-05).** Honzíkovo GO bylo na implementaci, výslovně NE na živý běh proti reálnému modelu. Nové soubory: `h2/extraction/prompt-content.ts` (DRAFT prompt, `v1-draft-2026-09-05a`, žádné Structured Outputs — bod 4a beze změny, `payload: z.record(...)` zůstává neslučitelný s `additionalProperties: false`), `h2/extraction/prompt-fixtures.ts` (10 fixtur — 4× happy_path, 2× malformed_input, 2× adversarial_context, 2× schema_validation, `expectedValid: true` u všech, stejná konvence jako BUDDY_RESPONSE fixtury — + `OPERATIONAL_EXTRACTION_DRY_RUN_MOCKS` jen pro dry-run větev), `h2/db/scripts/certify-operational-extraction-prompt.ts` (mirror `certify-buddy-response-prompt.ts` vzoru pro Haiku/extraction cestu: `createDraftPromptVersion` → `runPromptFixtureSuite` proti reálnému Haiku, vypíše výsledky; NIKDY nevolá `activatePromptVersion()` — aktivace zůstává oddělený ruční krok). `validateOutput` sdílí `OperationalExtractionOutputSchema` (`h2/extraction/operational-schema.ts`) přímo importem se stejným schématem, jaké validuje `extractOperationalCandidates()` v produkci — jediný zdroj pravdy o tvaru. Žádná migrace, žádný nový env.

`--dry-run` větev skriptu nepotřebuje žádný env ani DB připojení (ověřuje jen parse/validate cestu proti mockovaným výstupům) — spuštěno: **10/10 fixtur PASS**, žádné Anthropic volání, žádné DB připojení. `npx tsc --noEmit` a `npm run build` čisté, 284/284 testů v repu zelených (beze změny — žádný existující test se touhle prací nedotkl).

Odhad nákladu živého běhu (10 fixtur, systémový prompt ~782 tokenů/volání, `h2/context/token-budget.ts` heuristikou 3,5 znaku/token): input ~8 004 tokenů celkem (systémový prompt se posílá znovu při každém z 10 volání), output ~353 tokenů (optimisticky, odhad z velikosti mock výstupů) až ~20 480 tokenů (pesimisticky, `maxOutputTokens=2048` strop na fixturu) → **odhad $0,01–$0,11** při Haiku sazbě $1/$5 za MTok (pro srovnání: BUDDY_RESPONSE round 7, 12 fixtur na Sonnetu, stálo $0,1091). Živý běh proti reálnému Haiku **NEPROBĚHL** — čeká na Honzíkovo samostatné GO, až uvidí tenhle odhad.

**Nález (zapsáno 2026-09-05, neřešeno v tomhle kroku):** `h2/buddy/prompt-content.ts` a `h2/buddy/prompt-fixtures.ts` (obsah ACTIVE `BUDDY_RESPONSE` promptu, verze 7) existují jen na nemergnuté a dnes silně zastaralé větvi `prep/h2-buddy-response-prompt-draft` (diff proti `main`: 57 souborů, chybí jí celý BUILD-11 a kus BUILD-10) — na `main` tenhle obsah není. Produkční prompt text dnes žije jen jako řádek v `prompt_versions.content` v DB, ne v gitu. Riziko: budoucí re-certifikace (např. po změně modelu nebo revizi promptu) nemá co v gitu reprodukovat, jen DB obsah a starou nemergnutelnou větev. Samostatné rozhodnutí (mergnout obsah prep větve na `main` jako dokumentaci, nebo exportovat aktuální DB obsah do repa), neřešeno teď — jen zapsáno jako nález.
