...
3. **PR [#36](https://github.com/honzabindr-max/muj-web/pull/36) (tolerantní JSON extrakce) čeká na merge.** Zásah do už nasazeného BUILD-10 (`h2/buddy/parse-model-output.ts` + `generate-response.ts`) — Honzíkovo rozhodnutí 2026-09-04: prose-kolem-JSON selhání (3 z 5 certifikačních běhů na fixtuře "posuzování třetí osoby") se řeší deterministickou extrakcí, ne repair-retry. 232/232 testů, push+PR proběhly, merge čeká na GO.
4. **Structured Outputs (Anthropic API) — rozhodnuté prozkoumat, nepostaveno.** Zjištěno a napsáno Honzíkovi (2026-09-04): `output_config.format` je GA, funguje přes syrový fetch (žádná SDK závislost), oba naše modely (`claude-sonnet-5`, `claude-haiku-4-5-20251001`) jsou podporované. Pro BUDDY_RESPONSE by pravděpodobně odstranilo přesnou příčinu prose-kolem-JSON chyby (API garantuje `content[0]` je validní JSON přímo při generaci), ale JSON Schema má omezení (žádný `minLength`/`minItems`/číselné meze) — zod validace (AT-50) by zůstala nutná i tak. Pro `OPERATIONAL_EXTRACTION` (BUILD-08) nefunguje čistě — otevřený `payload: z.record(...)` vyžaduje `additionalProperties` jiné než `false`, což structured outputs nepodporuje; čeká na BUILD-12 rozhodnutí konkrétního tvaru. Doporučení Code: nechat PR #36 (tolerantní parser) jako defense-in-depth i po případném zavedení structured outputs, ne jedno druhým nahrazovat — rozhodnutí, jestli/kdy structured outputs zavést, je na Honzíkovi, nic z tohohle není postavené.
...
6. **Red team nálezy k zapsání.** Honzík zmínil (2026-09-04) dřívější odhady cen/limitů Anthropic API z red-teamu, které se ukázaly jako needs-verification ("neber jako fakt, ověř v dokumentaci"). Ověření proběhlo (viz bod 4 výše, Structured Outputs research, opřené o oficiální dokumentaci). Samotný red-team zdroj/dokument ale není v tomhle repu ani v konverzaci, co vedla k tomuhle zápisu, zaznamenaný — jen instrukce k ověření. Pokud má red-team review vzniknout jako trvalý záznam (např. nový DEC- zápis v DECISIONS.md), čeká na Honzíka, až dodá obsah k zapsání.

**Shrnutí závislostí:** 2 → potřebuje 3 dokončit? Ne — nezávislé (re-auth stránka a tolerantní parser se netýkají stejného kódu). Pořadí, ve kterém dává smysl mergovat: #37 (re-auth) → ruční aktivace DRAFT v6 (Honzíkův krok, vyžaduje prohlížeč) → #36 (tolerantní parser, kdykoliv nezávisle) → BUILD-11 (trigger, čeká na oboje + na body 4/5 rozhodnutí, pokud se do něj promítnou).
