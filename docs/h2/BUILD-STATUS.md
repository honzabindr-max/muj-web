...
| v2: INFO/EVENT podle aktéra, typ COMMAND, šifrování notes+commands (DEC-009 doplněk 2) | NASAZENO 2026-09-23 15:44 UTC — `pytest` 211/211, eval **41/41** (0,1219 USD), klíč varianta A (`H2IW_ENCRYPTION_KEY`, key_id `h2iw`) | [PR #56](https://github.com/honzabindr-max/muj-web/pull/56); migrace 1 poznámky: `notes` má jen `ciphertext`/`key_id`, dešifrování OK, plaintext v `items` vymazán; sken `state.db`/`-wal`/`-shm` na text poznámky = 0 výskytů (pozitivní kontrola na nešifrovaný TASK text = 1); záloha před migrací zničena `shred -u`; timer běží, další běhy `errors=0` |
| v3: COMMAND → projekt „H2 · Příkazy" (otevřený, beze změny), stavové aktualizace = COMMAND (DEC-009 doplněk 3) | viz Evidence v PR | `pytest` 232/232, eval **47/47** (0,1470 USD) |
| v4: nová věc + připomenutí = TASK s časem a Todoist push reminderem (DEC-009 doplněk 4) | viz PR | `pytest` 257/257, eval **53/53** (0,1731 USD) |
...
| v9: Planning OS v0.6 — 🚗 = účel, 📌 pevné, vazba blok↔úkol, test vlastnictví, došlý kredit = čekání (DEC-009 doplněk 9) | viz níže — `pytest` 446/446; eval `--full` **96/96** přes batch + cache, 0,0625 USD | [PR #63](https://github.com/honzabindr-max/muj-web/pull/63) |
| v9b: prompt caching (prefix 4 392 tok.), eval přes Batches API (`--changed`/`--full`), cena ze skutečného usage (DEC-009 doplněk 10) | viz PR #63 | cache ověřena (zápis 4392 → čtení 4392) |
