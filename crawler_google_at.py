#!/usr/bin/env python3
"""Google Suggest Crawler — Austria (hl=de, gl=at). Same architecture as Czech Google crawler."""
...
HL = "de"
GL = "at"
TABLE = "google_suggestions_at"
STATE_TABLE = "google_crawl_state_at"
...
def run(db, api):
    ...
    if state["status"] in ("idle", "completed"):
        print("  🆕 Starting fresh Google AT crawl from depth 0")
    ...
    if not state["queue"] and not state["next_queue"]:
        ...
        save_state(db, state)
        print("  🏁 Google AT crawl complete!")
        return
...
db = DB(url, key)
api = GoogleAPI(0.5)
print("🔍 Google Suggest Crawler (de, gl=at)")
