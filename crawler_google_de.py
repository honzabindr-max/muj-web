#!/usr/bin/env python3
"""Google Suggest Crawler — Germany (hl=de, gl=de). Same architecture as Czech Google crawler."""
...
MAX_RETRIES = 3
MAX_RUNTIME = 25 * 60
EXPAND_CHARS = list("abcdefghijklmnopqrstuvwxyzäöüß0123456789 ")
ALL_ROOTS = list("abcdefghijklmnopqrstuvwxyzäöüß0123456789")
SAFE = '=&.,()!*:"'

HL = "de"
GL = "de"
TABLE = "google_suggestions_de"
STATE_TABLE = "google_crawl_state_de"
...
class GoogleAPI:
    ...
    def fetch(s, phrase):
        ...
        params = urllib.parse.urlencode(
            {"client": "firefox", "q": phrase, "hl": HL, "gl": GL}
        )
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if e.code in (429, 503):
                    ...
                    print("  ⚠ HTTP " + str(e.code) + ", pokus " + str(attempt + 1))
                    time.sleep(2)
            except Exception as e:
                s.errs += 1
                print("  ⚠ " + str(e) + ", pokus " + str(attempt + 1))
                time.sleep(2)
        return []
...
def run(db, api):
    state = load_state(db)
    if not state:
        print("✗ No " + STATE_TABLE + " row")
        return
    if state["status"] in ("idle", "completed"):
        print("  🆕 Starting fresh Google DE crawl from depth 0")
    ...
    if not state["queue"] and not state["next_queue"]:
        ...
        save_state(db, state)
        print("  🏁 Google DE crawl complete!")
        return
...
db = DB(url, key)
api = GoogleAPI(0.5)
print("🔍 Google Suggest Crawler (de, gl=de)")
